import type { ConsoleUser, AuthResponse, CreateConsoleUserRequest, UpdateConsoleUserRequest } from '../types';
...
export const authApi = {  // Web Console Authentication & User Management
  ...
  setupAdmin: (username: string, password: string, displayName?: string) =>
	fetchJSON<{ status: string; user: ConsoleUser; warning?: string }>(`${BASE_URL}/auth/setup`, {
      method: 'POST',
  ...
  getMe: () =>
    fetchJSON<{ user: ConsoleUser }>(`${BASE_URL}/auth/me`),
  ...
  getConsoleUsers: () =>
		fetchJSON<{ users: ConsoleUser[] }>(`${BASE_URL}/auth/users`),
  createConsoleUser: (req: CreateConsoleUserRequest) =>
		fetchJSON<{ status: string; user: ConsoleUser }>(`${BASE_URL}/auth/users`, {
      method: 'POST',
      body: JSON.stringify(req),
    }),
  updateConsoleUser: (id: string, req: UpdateConsoleUserRequest) =>
		fetchJSON<{ status: string; user: ConsoleUser }>(`${BASE_URL}/auth/users/${id}`, {
      method: 'PUT',
      body: JSON.stringify(req),
    }),
  deleteConsoleUser: (id: string) =>
