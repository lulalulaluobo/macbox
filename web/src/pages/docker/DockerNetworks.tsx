      ...
      <div className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800/90 shadow-xl space-y-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Docker 镜像源加速器</h3>
              <p className="text-xs text-slate-400 mt-0.5">
                配置国内镜像源加速拉取，有效解决国内 Docker Hub 访问缓慢与超时问题
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold shadow-lg shadow-amber-500/20 transition disabled:opacity-50"
          >
        ...
        <div className="space-y-2">
          <span className="text-xs font-semibold text-slate-300">快速添加预设加速源:</span>
        ...
        <div className="space-y-2 pt-2">
          <span className="text-xs font-semibold text-slate-300">当前已生效加速列表:</span>
          {mirrors.length === 0 ? (
            <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 text-xs text-slate-500">
            ...
            <div className="space-y-2">
              {mirrors.map((m, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-3 rounded-2xl bg-slate-950/70 border border-slate-800 text-xs font-mono"
                >
          ...
          <div className="flex space-x-2 pt-1">
            <input
              ...
              className="flex-1 px-3.5 py-2 text-xs font-mono bg-slate-950 border border-slate-800 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 transition"
            />
            <button
      ...
      <div className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800/90 shadow-xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Docker 虚拟网络</h3>
              <p className="text-xs text-slate-400 mt-0.5">
                底层虚拟网卡桥接、Host直通与 Compose 独立子网拓扑
              </p>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-slate-800 overflow-hidden">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono">
              {networks.map(net => (
                <tr key={net.id} className="hover:bg-slate-800/30 transition">
                  <td className="py-3 px-4 font-bold text-white font-mono">{net.name}</td>
                  <td className="py-3 px-4 text-slate-400">{net.id.slice(0, 12)}</td>
                  <td className="py-3 px-4 text-sky-400">{net.driver}</td>
