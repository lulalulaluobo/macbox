...
import { Crown, Palette, Shield, Terminal, UserCheck, Users } from 'lucide-react';

export type SettingsSubTab = 'nas_users' | 'users' | 'rootpwd' | 'ssh' | 'terminal' | 'appearance';
...
const tabs: Array<{ id: SettingsSubTab; label: string; icon: React.ComponentType<{ className?: string }>; iconClassName?: string }> = [
  { id: 'nas_users', label: 'NAS 用户', icon: UserCheck },
...
export const SettingsNavigation: React.FC<SettingsNavigationProps> = ({ activeSubTab, onChange }) => (
  <div className="grid grid-cols-3 gap-1 rounded-2xl border border-slate-200 bg-slate-100/80 p-1.5 text-xs font-semibold dark:border-slate-800 dark:bg-slate-900/70 sm:grid-cols-6">
    {tabs.map(({ id, label, icon: Icon, iconClassName }) => (
      <button
        key={id}
        onClick={() => onChange(id)}
