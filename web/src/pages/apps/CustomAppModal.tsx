...
export const CustomAppModal: React.FC<CustomAppModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl h-[85vh] flex flex-col rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="px-6 py-4 bg-slate-950/70 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-2 rounded-xl bg-gradient-to-r from-amber-500/20 to-orange-500/20 text-amber-400 border border-amber-500/30">
              <Plus className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-bold text-base text-white">导入自定义应用 (Docker Compose)</h3>
              <p className="text-xs text-slate-400">
                将任意 Docker Compose 项目以标准应用形式收录入应用商城
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 p-6 overflow-y-auto space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">
                应用显示名称 <span className="text-rose-400">*</span>
              </label>
              <input
                type="text"
                required
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-800 text-white font-mono text-xs focus:outline-none focus:border-amber-500"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">应用分类</label>
              <select
                value={category}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">默认 WebUI 端口</label>
              <input
                type="number"
                value={port}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">图标样式</label>
              <select
                value={icon}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">应用简介说明</label>
            <input
              type="text"
          ...
          <div className="space-y-1.5 flex-1 flex flex-col min-h-[220px]">
            ...
            <textarea
              ...
              className="flex-1 w-full p-3 font-mono text-xs text-emerald-400/90 bg-[#06090e] border border-slate-800 rounded-2xl resize-none focus:outline-none focus:border-amber-500 leading-relaxed"
        ...
        <div className="px-6 py-4 bg-slate-950/70 border-t border-slate-800 flex items-center justify-end space-x-3">
          ...
          <button
            ...
            disabled={saving}
            className="flex items-center space-x-2 px-5 py-2 rounded-xl bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white text-xs font-bold shadow-lg shadow-amber-500/20 transition disabled:opacity-50"
          >
